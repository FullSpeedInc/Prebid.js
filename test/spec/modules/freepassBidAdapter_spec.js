...
import {spec} from 'modules/freepassBidAdapter';
  ...
  describe('buildRequests', function () {
    ...
    beforeEach(function () {
      bidRequests = [{
        "bidder": "freepass",
        "params": {
          "bidServiceUrl": "http://localhost:8080/bid",
        },
        "userId": {
          "freepassId": {
            "userIp": "172.21.0.1",
    ...
    it('should return a valid bid request object', function () {
      ...
      expect(bidRequest.url).to.equal('http://localhost:8080/bid');
    });
    ...
    it('should fallback to default bidder service URL when bidServiceUrl is missing', function () {
      let localBidRequests = [Object.assign({}, bidRequests[0])];
      delete localBidRequests[0].params;
      const bidRequest = spec.buildRequests(localBidRequests, bidderRequest);
      expect(bidRequest.url).to.be.a('string');
      expect(bidRequest.url).to.be.not.empty;
    });
  });
